(function(){
  ...
  (function(){
    ...
    (function frame(){
      if(built && len){
        if(reduced){ cur = 1; }
        else{ computeTarget(); cur += (targetP - cur) * 0.05; }
        maskPath.setAttribute('stroke-dashoffset', String(len * (1 - cur)));
      }
      requestAnimationFrame(frame);
    })();
  ...
  // philanthropy hero — red lightning shader behind the 1% stat (philanthropy.html only)
  (function(){
    var hero = document.getElementById('philoHero');
    var canvas = document.getElementById('lightningCanvas');
    if(!hero || !canvas) return;

    // fixed chrome floats over the always-black hero — keep it white up there
    function overHero(){
      document.body.classList.toggle('over-hero', window.scrollY < hero.offsetHeight - 70);
    }
    overHero();
    ...
    var vsSrc = 'attribute vec2 aPosition;void main(){gl_Position=vec4(aPosition,0.0,1.0);}';
    var fsSrc =
      ...
      'for(int i=0;i<OCTAVE_COUNT;++i){v+=amp*noise(p);p*=rotate2d(0.45);p*=2.0;amp*=0.5;}return v;}' +
      'void main(){' +
      'vec2 uv=gl_FragCoord.xy/iResolution.xy;uv=2.0*uv-1.0;uv.x*=iResolution.x/iResolution.y;' +
      'uv+=2.0*fbm(uv*1.8+0.8*iTime*1.2)-1.0;' +
      'float dist=abs(uv.x);' +
      'vec3 baseColor=hsv2rgb(vec3(0.0,0.9,0.8));' +
      'vec3 col=baseColor*pow(mix(0.0,0.07,hash11(iTime*1.2))/dist,1.0)*0.7;' +
